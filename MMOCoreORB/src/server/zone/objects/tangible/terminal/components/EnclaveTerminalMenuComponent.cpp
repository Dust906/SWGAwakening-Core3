...
void EnclaveTerminalMenuComponent::fillObjectMenuResponse(SceneObject* sceneObject, ObjectMenuResponse* menuResponse, CreatureObject* player) const {
	...
	if (terminalType == VOTING) {
		menuResponse->addRadialMenuItem(20, 3, "Voting");
		menuResponse->addRadialMenuItemToRadialID(20, 21, 3, "@force_rank:vote_status"); // Voting Status
		menuResponse->addRadialMenuItemToRadialID(20, 22, 3, "@force_rank:petition"); // Petition
		menuResponse->addRadialMenuItemToRadialID(20, 23, 3, "@force_rank:record_vote"); // Record Vote
		menuResponse->addRadialMenuItemToRadialID(20, 24, 3, "@force_rank:accept_promotion"); // Accept Promotion

		//if (playerRank > 7 && enclaveType == FrsManager::COUNCIL_LIGHT)
		if (playerRank > 7)
			menuResponse->addRadialMenuItem(25, 3, "@force_rank:demote_member"); // Demote Lower Tier Member

		menuResponse->addRadialMenuItem(26, 3, "@force_rank:recover_jedi_items"); // Recover Jedi Items

		if (ghost->isPrivileged())
			menuResponse->addRadialMenuItem(27, 3, "Force Phase Change");
	}
}
...
int EnclaveTerminalMenuComponent::handleObjectMenuSelect(SceneObject* sceneObject, CreatureObject* player, byte selectedID) const {
	...
	if (terminalType == VOTING && (selectedID >= 21 && selectedID <= 27)) {
		if (selectedID == 21)
		...
		else if (selectedID == 27 && ghost->isPrivileged())
